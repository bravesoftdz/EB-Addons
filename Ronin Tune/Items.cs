    ...
    class Items
    {
        ...
        private static void UseItem1(Item item)
        {
            if (!(Player.Instance.CountEnemiesInRange(1500) >= 1)
                || !ItemMenu["item1"].Cast<CheckBox>().CurrentValue
                || !(ItemMenu["item1MyHp"].Cast<Slider>().CurrentValue < Player.Instance.HealthPercent))
            {
                return;
            }
        ...
        private static void UseItem2(Item item)
        {
            if (!(Player.Instance.CountEnemiesInRange(1000) >= 1)
                || !ItemMenu["item2"].Cast<CheckBox>().CurrentValue
                || ItemMenu["item2MyHp"].Cast<Slider>().CurrentValue < Player.Instance.HealthPercent)
            {
                return;
            }
        ...
        private static void UseItem3(Item item, AIHeroClient target)
        {
            if (!target.IsValidTarget(550)
                || !ItemMenu["item3"].Cast<CheckBox>().CurrentValue
                || ItemMenu["item34EnemyHP"].Cast<Slider>().CurrentValue < target.HealthPercent)
            {
                return;
            }
        ...
        private static void UseItem4(Item item, AIHeroClient target)
        {
            if (!target.IsValidTarget(700)
                || !ItemMenu["item4"].Cast<CheckBox>().CurrentValue
                || ItemMenu["item34EnemyHP"].Cast<Slider>().CurrentValue < target.HealthPercent)
            {
                return;
            }
        ...
        {
            if (!(Player.Instance.CountEnemiesInRange(500) >= ItemMenu["item5Enemy"].Cast<Slider>().CurrentValue)
                || !ItemMenu["item5"].Cast<CheckBox>().CurrentValue)
            {
                return;
            }
        ...
        private static void UseItem6(Item item)
        {
            if (!(Player.Instance.CountEnemiesInRange(Player.Instance.MoveSpeed * 2) >= 1)
                || !ItemMenu["item6"].Cast<CheckBox>().CurrentValue
                || ItemMenu["item6Ally"].Cast<Slider>().CurrentValue > Player.Instance.CountAlliesInRange(500))
            {
                return;
            }
        ...
        {
            if (ItemMenu["item7Ally"].Cast<Slider>().CurrentValue > Player.Instance.CountAlliesInRange(600)
                || !ItemMenu["item7"].Cast<CheckBox>().CurrentValue)
            {
                return;
            }
